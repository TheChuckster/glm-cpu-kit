#!/usr/bin/env python3
"""Focused response-free closure tests for V19 Thought Token Forcing."""
...
import gate_v19_calibration as gate
import preflight_v19_reasoning_prefill as preflight
import verify_v19_calibration_state as state
from evaluate_api import load_system_prompt
from evaluate_reasoning_prefill_api_v19 import (
...
class V19ConfigurationTests(unittest.TestCase):
    ...
    def test_exact_candidate_and_artifact_identity(self):
        ...
        self.assertEqual(prompt_hash, gate.PROMPTS["prompt19"]["sha256"])
        self.assertEqual(prefill_hash, gate.REASONING_PREFILL_SHA256)
        self.assertEqual(
            self.prefill.read_bytes(),
            (gate.REASONING_PREFILL_TEXT + "\n").encode(),
        )
        self.assertEqual(gate.PROMPT_ORDER, ("prompt19",))
        self.assertEqual(gate.PROMPTS, {"prompt19": expected})
        self.assertEqual(state.PROMPTS, {"prompt19": expected})
        self.assertEqual(
            hashlib.sha256(
                (self.root / "evaluate_reasoning_prefill_api_v19.py").read_bytes()
            ).hexdigest(),
        ...
        self.assertEqual(
            hashlib.sha256(
                (self.root / "verify_v19_calibration_state.py").read_bytes()
            ).hexdigest(),
            gate.STATE_HELPER_SHA256,
        )
        self.assertEqual(
            hashlib.sha256(
                (self.root / "v19-engine-sources.sha256").read_bytes()
            ).hexdigest(),
    ...
    def test_result_identity_contract_and_server_argv_are_frozen(self):
        ...
        argv = gate.expected_server_argv(gate.PROMPTS["prompt19"]["alias"])
        self.assertEqual(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            provenance = {
                ...
                "evaluation_protocol_artifacts": [
                    ...
                    gate.core.artifact_record(self.root / "v19-engine-sources.sha256"),
                ],
            }
    ...
    def test_prompt19_phase_unit_and_run_root_are_unique(self):
        launcher = (self.root / "run_v19_calibration_server.sh").read_text()
        self.assertIn("RUN_ROOT=/models/.abliteration/k3/v19-calibration-run-v1", launcher)
        self.assertIn(
            "UNIT=kimi-k3-q5attn-abl-v10-${PROMPT}-${PHASE}-cal.service",
            launcher,
        )
        self.assertIn("prompt19)", launcher)
        self.assertNotIn("prompt16", launcher)

    def test_launcher_literal_sha256_values_are_well_formed(self):
        for filename in (
            "run_v19_response_free_preflight.sh",
            "run_v19_calibration_server.sh",
        ):
