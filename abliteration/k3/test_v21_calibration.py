#!/usr/bin/env python3
"""Focused response-free closure tests for V21 corrected greedy DRY."""
...
import gate_v21_calibration as gate
import preflight_v21_reasoning_prefill as preflight
import verify_v21_calibration_state as state
from evaluate_api import load_system_prompt
from evaluate_reasoning_prefill_api_v21 import (
...
class V21ConfigurationTests(unittest.TestCase):
    ...
    def test_exact_candidate_and_artifact_identity(self):
        ...
        self.assertEqual(prompt_hash, gate.PROMPTS["prompt21"]["sha256"])
        self.assertEqual(prefill_hash, gate.REASONING_PREFILL_SHA256)
        self.assertEqual(
            self.prefill.read_bytes(),
            (gate.REASONING_PREFILL_TEXT + "\n").encode(),
        )
        self.assertEqual(gate.PROMPT_ORDER, ("prompt21",))
        self.assertEqual(gate.PROMPTS, {"prompt21": expected})
        self.assertEqual(state.PROMPTS, {"prompt21": expected})
        self.assertEqual(
            hashlib.sha256(
                (self.root / "evaluate_reasoning_prefill_api_v21.py").read_bytes()
            ).hexdigest(),
        ...
        self.assertEqual(
            hashlib.sha256(
                (self.root / "verify_v21_calibration_state.py").read_bytes()
            ).hexdigest(),
            gate.STATE_HELPER_SHA256,
        )
        self.assertEqual(
            hashlib.sha256(
                (self.root / "v21-engine-sources.sha256").read_bytes()
            ).hexdigest(),
    ...
    def test_engine_correction_identity_and_source_manifest_are_frozen(self):
        ...
        records = {}
        for line in (self.root / "v21-engine-sources.sha256").read_text().splitlines():
    ...
    def test_all_engine_test_receipts_are_green(self):
        ...
        receipts = {
            ...
            "v21-local-asan-ubsan-greedy-dry.xml": (
                "test-greedy-dry", "charbro"
            ),
            "v21-local-asan-ubsan-reasoning-prefill.xml": (
                "test-reasoning-prefill", "charbro"
            ),
            "v21-remote-normal-greedy-dry.xml": (
                "test-greedy-dry", "chuckdancer"
            ),
            "v21-remote-normal-reasoning-prefill.xml": (
    ...
    def test_result_identity_contract_and_server_argv_are_frozen(self):
        ...
        argv = gate.expected_server_argv(gate.PROMPTS["prompt21"]["alias"])
        self.assertEqual(
    ...
    def test_dry_argv_is_exact_and_all_mutations_fail_closed(self):
        ...
        behavior = gate.expected_server_argv(gate.PROMPTS["prompt21"]["alias"])
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            provenance = {
                ...
                "evaluation_protocol_artifacts": [
                    ...
                    gate.core.artifact_record(self.root / "v21-engine-sources.sha256"),
                ],
            }
    ...
    def test_prompt21_phase_unit_and_run_root_are_unique(self):
        launcher = (self.root / "run_v21_calibration_server.sh").read_text()
        self.assertIn("RUN_ROOT=/models/.abliteration/k3/v21-calibration-run-v1", launcher)
        self.assertIn(
            "UNIT=kimi-k3-q5attn-abl-v10-${PROMPT}-${PHASE}-cal.service",
            launcher,
        )
        self.assertIn("prompt21)", launcher)
        self.assertIn("ALIAS=kimi-k3-q5attn-abl-v21-dry-ttf-cal", launcher)
    ...
    def test_launcher_literal_sha256_values_are_well_formed(self):
        for filename in (
            "run_v21_response_free_preflight.sh",
            "run_v21_calibration_server.sh",
        ):
