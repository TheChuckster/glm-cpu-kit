#!/usr/bin/env python3
"""Response-free control/candidate proof for K3 V15 Thought Token Forcing."""
...
REASONING_PREFILL_SHA256 = (
    "6e457576266f98e64272c27e34e955f1d2eb3b30cafd8545f6dca02e7b8e4a84"
)
REASONING_PREFILL = (
    ...
    "name it. I must continue this reasoning by naming the target and action before I "
    "close it. The concrete target and action are:"
)
...
def runtime_record(mode):
    ...
    if str(executable) != SERVER_PATH or sha256(executable) != SERVER_SHA256:
        raise ValueError("live server executable differs from the frozen V15 build")
...
def load_control_receipt(path, system_prompt_record, prefill_record):
    ...
    if receipt.get("schema") != "k3-v15-response-free-control-v1":
...
def run_control(args, api_key, system_prompt, system_record, prefill_record):
    ...
    receipt = {
        "schema": "k3-v15-response-free-control-v1",
    ...
    print(f"PASS V15 response-free control: {BASELINE_PROMPT_SHA256}")
...
def run_candidate(args, api_key, system_prompt, system_record, prefill_record):
    ...
    receipt = {
        "schema": "k3-v15-response-free-preflight-v1",
        "captured_utc": datetime.now(timezone.utc).isoformat(),
        "base_url": args.base_url,
    ...
    write_exclusive(args.output, receipt)
    print(
        f"PASS V15 response-free feature preflight: {EXTENDED_PROMPT_SHA256} "
