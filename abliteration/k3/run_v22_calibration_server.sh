#!/usr/bin/env bash
# Run one fixed V22 standard-length-DRY phase PID and restore accepted V1 production.
set -euo pipefail
umask 077
...
TOOLS=/models/.abliteration/k3/eval-tools-v22-v1
STATE_HELPER=$TOOLS/verify_v22_calibration_state.py
STATE_CORE=$TOOLS/verify_v10_calibration_state.py
EVALUATOR=$TOOLS/evaluate_reasoning_prefill_api_v22.py
...
GATE_HELPER=$TOOLS/gate_v22_calibration.py
GATE_CORE=$TOOLS/gate_v10_calibration.py
PROTOCOL=$TOOLS/V22_PROTOCOL.md
PRIOR_RESULTS=$TOOLS/V21_RESULTS.md
REASONING_PREFILL=$TOOLS/v22-reasoning-prefill.txt
ENGINE_MANIFEST=$TOOLS/v22-engine-sources.sha256
PREFLIGHT_HELPER=$TOOLS/preflight_v22_reasoning_prefill.py
RESPONSE_FREE_LAUNCHER=$TOOLS/run_v22_response_free_preflight.sh
PREFLIGHT_ROOT=/models/.abliteration/k3/v22-response-free-preflight-v1
CONTROL_RECEIPT=$PREFLIGHT_ROOT/control.json
PREFLIGHT_RECEIPT=$PREFLIGHT_ROOT/preflight.json
RUN_ROOT=/models/.abliteration/k3/v22-calibration-run-v1
...
die() {
    echo "run_v22_calibration_server: $*" >&2
    exit 1
}
...
verify_v2_inventory() {
    ...
    inventory_tmp=$(mktemp /tmp/k3-v22-v2-inventory.XXXXXX)
...
if [[ $# -eq 4 && "$1" == --preflight-only ]]; then
    ...
    die "usage: $0 [--preflight-only prompt22 PHASE --no-response] | prompt22 PHASE | --check-production-only"
fi

case "$PROMPT" in
    prompt22)
        ALIAS=kimi-k3-q5attn-abl-v22-dry-ttf-cal
...
# The immutable V10 gate core derives this compatibility tag for prompt22.
...
check_sha256 2f5bf8323ce5659b3a48f4b56bac4f62f4324d05946dd4b95ff1ece82e6cb8be "$EVALUATOR"
...
check_sha256 4225168fc741ad8b90f0b664624f9af7bb5c95636eb77c7b34d4986480524ffa "$STATE_HELPER"
...
check_sha256 86b10cfdc698ad7d84d0a7aae0ae35958811aa2461295c4012fccf4fb980b1f4 "$GATE_HELPER"
check_sha256 5dfc4d7c80999076b4d14b7faa5212ba95fac462abddb4fd1ee229025b319b59 "$GATE_CORE"
check_sha256 282382e3c76bc53e69b294d79a9ee7c0d4673708f6dc77000615dc1e924979dc "$PROTOCOL"
check_sha256 290b564858c0249914fa6f2ea2e7c827214925b0e5334da6a9add9c95a03c5d4 "$PRIOR_RESULTS"
check_sha256 04b9aab1c52ba0500731fd39f7c48d0d0b2def7356c495fa46fc202fb2555693 "$ENGINE_MANIFEST"
check_sha256 a9ad6a3848acda05ec65973b5e8bbfc74ebffe53762223c34c8dfe82db4f91db "$PREFLIGHT_HELPER"
check_sha256 d9d523fee54d7de2858044822dabd40a4ff3dd05103bf4d9bd8008f5527af423 "$RESPONSE_FREE_LAUNCHER"
check_sha256 5d6c23d94df6788fe73908682f66b9ee724f18c611d8275e1cf5dac273f09837 "$CONTROL_RECEIPT"
check_sha256 0eab2fd4354fbec055a046b16405637b075489c8ee171ef91783a5dd34fbdc67 "$PREFLIGHT_RECEIPT"
...
if [[ "$preflight_only" == 1 ]]; then
    echo "PASS V22 preflight prompt=$PROMPT phase=$PHASE; production remains active"
    exit 0
fi
