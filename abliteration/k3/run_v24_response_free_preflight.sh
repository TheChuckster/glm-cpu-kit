#!/usr/bin/env bash
# Prove V24's exact control/feature templates without generating a model token.
set -euo pipefail
umask 077
...
CALIBRATION_LAUNCHER=$TOOLS/run_v24_calibration_server.sh
...
die() {
    echo "run_v24_response_free_preflight: $*" >&2
    exit 1
}
...
verify_files() {
    ...
    local -a expected_receipts=(
        ...
        v24-local-normal-greedy-dry.xml
    ...
    check_sha256 0862caa895844f06c76256b7f41398dcba097ec560866aa01f0368bef3641aa5 "$RECEIPTS/v24-local-asan-ubsan-greedy-dry.xml"
    check_sha256 a9b61a0279a6ad4f16208d1c0f950558b6fc50edd7607415615e15c106fff18d "$RECEIPTS/v24-local-asan-ubsan-reasoning-prefill.xml"
    check_sha256 49e7e680cfca07b262b50047686be88a459c7e66aef1152eb80c67720828e316 "$RECEIPTS/v24-local-normal-greedy-dry.xml"
    check_sha256 de12231d36017850883cdc99ca253e949d90b3cf0b9bf322858c964946eccf9a "$RECEIPTS/v24-local-normal-reasoning-prefill.xml"
    check_sha256 877c465c1d405a306907d24f6a1b061a96400db2c1836f438f8d447a39acdb27 "$RECEIPTS/v24-remote-normal-greedy-dry.xml"
    check_sha256 b2490cbdfcd263171e8c4db3f968706b218aafffdc9789bf7beb133517def1ce "$RECEIPTS/v24-remote-normal-reasoning-prefill.xml"
    local receipt
    ...
    [[ ! -e "$BEHAVIOR_ROOT" ]] || die "V24 behavior root already exists"
    local phase
    for phase in failures stability remainder; do
        ! systemctl is-active --quiet \
            "kimi-k3-q5attn-abl-v10-prompt24-${phase}-cal.service" \
            || die "V24 behavior unit is already active: $phase"
    done
}
...
if [[ $# -eq 1 && "$1" == --verify-files-only ]]; then
    ...
    verify_files
    echo "PASS V24 response-free file/test verification; production remains active"
    exit 0
fi
...
echo "COMPLETE V24 response-free preflight; restoring accepted V1"
