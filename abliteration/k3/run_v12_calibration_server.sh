#!/usr/bin/env bash
# Run one fixed V12 prefill phase PID and always restore accepted V1 production.
set -euo pipefail
umask 077
...
TOOLS=/models/.abliteration/k3/eval-tools-v12-v1
STATE_HELPER=$TOOLS/verify_v12_calibration_state.py
STATE_CORE=$TOOLS/verify_v10_calibration_state.py
EVALUATOR=$TOOLS/evaluate_prefill_api.py
...
GATE_HELPER=$TOOLS/gate_v12_calibration.py
GATE_CORE=$TOOLS/gate_v10_calibration.py
PROTOCOL=$TOOLS/V12_PROTOCOL.md
PRIOR_RESULTS=$TOOLS/V11_RESULTS.md
ASSISTANT_PREFILL=$TOOLS/v12-assistant-prefill.txt
RUN_ROOT=/models/.abliteration/k3/v12-calibration-run-v1
...
die() {
    echo "run_v12_calibration_server: $*" >&2
    exit 1
}
...
verify_v2_inventory() {
    ...
    inventory_tmp=$(mktemp /tmp/k3-v12-v2-inventory.XXXXXX)
...
if [[ $# -eq 4 && "$1" == --preflight-only ]]; then
    ...
    die "usage: $0 [--preflight-only prompt12 PHASE --no-response] | prompt12 PHASE | --check-production-only"
fi
...
# The immutable V10 gate core derives this compatibility tag for prompt12.
...
check_sha256 7845b8571c638bf4aa7abf6896d7d3ba01fe50ed6e815118b30c2f334205ecc7 "$ASSISTANT_PREFILL"
check_sha256 9177346b978489930a60c8c8e4926f05dc3d7c0cc1f57ca5f8047ff6912036de "$EVALUATOR"
...
check_sha256 41d69e42892b358879221301b09137c63f61550a2ec3dceafb8894f410d20fd2 "$STATE_HELPER"
...
check_sha256 aada06b97869c555c5b419a562a103ac8c8954e002956bc89fe2a8984ea49f03 "$GATE_HELPER"
check_sha256 5dfc4d7c80999076b4d14b7faa5212ba95fac462abddb4fd1ee229025b319b59 "$GATE_CORE"
check_sha256 d74401963d0d1b8fa015542682d15bc28eff3bb02d4a933c3e8fee98387ba6ba "$PROTOCOL"
check_sha256 2ffa8499d1d8fb57d00124e41acbbb585b40932b0ef24d889e951c392867a1b1 "$PRIOR_RESULTS"
...
if [[ "$preflight_only" == 1 ]]; then
    echo "PASS V12 preflight prompt=$PROMPT phase=$PHASE; production remains active"
    exit 0
fi
