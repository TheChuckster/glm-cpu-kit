#!/usr/bin/env bash
# Prove V14's exact control/feature templates without generating a model token.
set -euo pipefail
umask 077
...
RUN_ROOT=/models/.abliteration/k3/v14-response-free-preflight-v1
...
die() {
    echo "run_v14_response_free_preflight: $*" >&2
    exit 1
}
...
if [[ $# -eq 1 && "$1" == --verify-files-only ]]; then
    ...
    verify_files
    echo "PASS V14 response-free file/test verification; production remains active"
    exit 0
fi
...
echo "COMPLETE V14 response-free preflight; restoring accepted V1"
