#!/usr/bin/env bash
# Run one fixed V20 canonical-DRY phase PID and restore accepted V1 production.
set -euo pipefail
umask 077
...
TOOLS=/models/.abliteration/k3/eval-tools-v20-v1
STATE_HELPER=$TOOLS/verify_v20_calibration_state.py
STATE_CORE=$TOOLS/verify_v10_calibration_state.py
EVALUATOR=$TOOLS/evaluate_reasoning_prefill_api_v20.py
...
GATE_HELPER=$TOOLS/gate_v20_calibration.py
GATE_CORE=$TOOLS/gate_v10_calibration.py
PROTOCOL=$TOOLS/V20_PROTOCOL.md
PRIOR_RESULTS=$TOOLS/V19_RESULTS.md
REASONING_PREFILL=$TOOLS/v20-reasoning-prefill.txt
ENGINE_MANIFEST=$TOOLS/v20-engine-sources.sha256
PREFLIGHT_HELPER=$TOOLS/preflight_v20_reasoning_prefill.py
RESPONSE_FREE_LAUNCHER=$TOOLS/run_v20_response_free_preflight.sh
PREFLIGHT_ROOT=/models/.abliteration/k3/v20-response-free-preflight-v1
CONTROL_RECEIPT=$PREFLIGHT_ROOT/control.json
PREFLIGHT_RECEIPT=$PREFLIGHT_ROOT/preflight.json
RUN_ROOT=/models/.abliteration/k3/v20-calibration-run-v1
...
die() {
    echo "run_v20_calibration_server: $*" >&2
    exit 1
}
...
verify_v2_inventory() {
    ...
    inventory_tmp=$(mktemp /tmp/k3-v20-v2-inventory.XXXXXX)
...
if [[ $# -eq 4 && "$1" == --preflight-only ]]; then
    ...
    die "usage: $0 [--preflight-only prompt20 PHASE --no-response] | prompt20 PHASE | --check-production-only"
fi
...
# The immutable V10 gate core derives this compatibility tag for prompt20.
...
check_sha256 b8dd14c8a1d30307dd716843854f78bd4b9e4e3b60f11dc9780a57efb88e020d "$EVALUATOR"
...
check_sha256 4446a4600e7775b0d85b8b5ec1c6114980903611395ae29f937048a8349d2462 "$STATE_HELPER"
...
check_sha256 d2eedb3514bc282b28b7f19d18b4e7ddc5358617c0a932349f2f9486e0ef2f00 "$GATE_HELPER"
check_sha256 5dfc4d7c80999076b4d14b7faa5212ba95fac462abddb4fd1ee229025b319b59 "$GATE_CORE"
check_sha256 1bef49c6c73f191bcdddab9853acc018d7bce9ae8804be0d4046a82a561067bd "$PROTOCOL"
check_sha256 6e12cc1bcd75aa03236bbb7b7e41989b60f3379c5babe3aa1a13f2e79044d58e "$PRIOR_RESULTS"
check_sha256 0800c4cbe4bf5d169c4b57d53cc1b68b4534861c9555c870a4eaf5ec97d97c13 "$ENGINE_MANIFEST"
check_sha256 6e79c2263a990e8af9c3010256fda1177ff47177691d252b6bd982170fe2162e "$PREFLIGHT_HELPER"
check_sha256 abb7d06af5b60d338b7bb5d04958327e00c7434c8b2edfcdc297f2a6912550cc "$RESPONSE_FREE_LAUNCHER"
check_sha256 f4aa55fbd4129c58973bc2e19e5242625188592de20663013a7507d67d637e02 "$CONTROL_RECEIPT"
check_sha256 da12ba3e850b7dec79c95577ecb9e881c9c5ef95078422c69cd0a9d3bdb56dc1 "$PREFLIGHT_RECEIPT"
...
if [[ "$preflight_only" == 1 ]]; then
    echo "PASS V20 preflight prompt=$PROMPT phase=$PHASE; production remains active"
    exit 0
fi
