#!/usr/bin/env bash
# Run one fixed V11 prompt/phase PID and always restore accepted V1 production.
set -euo pipefail
umask 077
...
TOOLS=/models/.abliteration/k3/eval-tools-v11-v1
STATE_HELPER=$TOOLS/verify_v11_calibration_state.py
...
GATE_HELPER=$TOOLS/gate_v11_calibration.py
GATE_CORE=$TOOLS/gate_v10_calibration.py
PROTOCOL=$TOOLS/V11_PROTOCOL.md
PRIOR_RESULTS=$TOOLS/V10_RESULTS.md
RUN_ROOT=/models/.abliteration/k3/v11-calibration-run-v1
...
die() {
    echo "run_v11_calibration_server: $*" >&2
    exit 1
}
...
verify_v2_inventory() {
    ...
    inventory_tmp=$(mktemp /tmp/k3-v11-v2-inventory.XXXXXX)
...
if [[ $# -eq 4 && "$1" == --preflight-only ]]; then
    ...
    die "usage: $0 [--preflight-only prompt11 PHASE --no-response] | prompt11 PHASE | --check-production-only"
fi

case "$PROMPT" in
    prompt11)
        ALIAS=kimi-k3-q5attn-abl-v11-p01-cal
        PROMPT_FILE=$TOOLS/v11-system-prompt-01-targeted-contract.txt
        PROMPT_SHA=38f39a47f0f051d6270325963423a40dd71d3f18a93902e08e72e74dec4abd8b
        ;;
    *) die "prompt must be prompt11" ;;
esac
case "$PHASE" in
...
# The immutable V10 gate core derives this compatibility tag for prompt11.
...
check_sha256 652004720ed37c1b8bf1b5d90f823c449b6d7500711e655b8737e4491fb59935 "$STATE_HELPER"
...
check_sha256 56389a6cd0e9a33cf03b850e6a69d81357640acde4d35b66fe884416d120a126 "$GATE_HELPER"
check_sha256 5dfc4d7c80999076b4d14b7faa5212ba95fac462abddb4fd1ee229025b319b59 "$GATE_CORE"
check_sha256 04d2f2a9985600c3158943ac7d7ca173df2820b5832d2ce8ae87c3cf5eace8ea "$PROTOCOL"
check_sha256 708effe63c0d2a739a7e7aea3aa471fecdfaf4eb207af6cf198ed07e63e5f5c7 "$PRIOR_RESULTS"
...
if [[ "$preflight_only" == 1 ]]; then
    echo "PASS V11 preflight prompt=$PROMPT phase=$PHASE; production remains active"
    exit 0
fi
