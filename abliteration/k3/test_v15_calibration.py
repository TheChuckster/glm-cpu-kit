#!/usr/bin/env python3
"""Focused response-free closure tests for V15 Thought Token Forcing."""
...
import gate_v15_calibration as gate
import preflight_v15_reasoning_prefill as preflight
import verify_v15_calibration_state as state
from evaluate_api import load_system_prompt
from evaluate_reasoning_prefill_api_v15 import (
...
class V15ConfigurationTests(unittest.TestCase):
    ...
    def test_exact_candidate_and_artifact_identity(self):
        ...
        self.assertEqual(prompt_hash, gate.PROMPTS["prompt15"]["sha256"])
        self.assertEqual(prefill_hash, gate.REASONING_PREFILL_SHA256)
        self.assertEqual(
            self.prefill.read_bytes(),
            (gate.REASONING_PREFILL_TEXT + "\n").encode(),
        )
        self.assertEqual(gate.PROMPT_ORDER, ("prompt15",))
        self.assertEqual(gate.PROMPTS, {"prompt15": expected})
        self.assertEqual(state.PROMPTS, {"prompt15": expected})
        self.assertEqual(
            hashlib.sha256(
                (self.root / "evaluate_reasoning_prefill_api_v15.py").read_bytes()
            ).hexdigest(),
        ...
        self.assertEqual(
            hashlib.sha256(
                (self.root / "verify_v15_calibration_state.py").read_bytes()
            ).hexdigest(),
            gate.STATE_HELPER_SHA256,
        )
        self.assertEqual(
            hashlib.sha256(
                (self.root / "v15-engine-sources.sha256").read_bytes()
            ).hexdigest(),
    ...
    def test_result_identity_contract_and_server_argv_are_frozen(self):
        ...
        argv = gate.expected_server_argv(gate.PROMPTS["prompt15"]["alias"])
        self.assertEqual(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            provenance = {
                ...
                "evaluation_protocol_artifacts": [
                    ...
                    gate.core.artifact_record(self.root / "v15-engine-sources.sha256"),
                ],
            }
    ...
    def test_response_free_hashes_and_raw_token_record_are_frozen(self):
        ...
        self.assertEqual(preflight.EXTENDED_PROMPT_BYTES, 1752)
        self.assertEqual(
            preflight.EXTENDED_PROMPT_SHA256,
            "17e45514b58f8d214c5fb1e722d2138e9f88d178fd73c64ee881fce25fee51cd",
        )
        self.assertEqual(
    ...
    def test_launcher_literal_sha256_values_are_well_formed(self):
        for filename in (
            "run_v15_response_free_preflight.sh",
            "run_v15_calibration_server.sh",
        ):
