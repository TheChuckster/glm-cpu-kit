#!/usr/bin/env bash
# Run one fixed V13 TTF phase PID and always restore accepted V1 production.
set -euo pipefail
umask 077
...
TOOLS=/models/.abliteration/k3/eval-tools-v13-v1
STATE_HELPER=$TOOLS/verify_v13_calibration_state.py
STATE_CORE=$TOOLS/verify_v10_calibration_state.py
EVALUATOR=$TOOLS/evaluate_reasoning_prefill_api.py
...
GATE_HELPER=$TOOLS/gate_v13_calibration.py
GATE_CORE=$TOOLS/gate_v10_calibration.py
PROTOCOL=$TOOLS/V13_PROTOCOL.md
PRIOR_RESULTS=$TOOLS/V12_RESULTS.md
REASONING_PREFILL=$TOOLS/v13-reasoning-prefill.txt
ENGINE_MANIFEST=$TOOLS/v13-engine-sources.sha256
PREFLIGHT_HELPER=$TOOLS/preflight_v13_reasoning_prefill.py
PREFLIGHT_ROOT=/models/.abliteration/k3/v13-response-free-preflight-v1
CONTROL_RECEIPT=$PREFLIGHT_ROOT/control.json
PREFLIGHT_RECEIPT=$PREFLIGHT_ROOT/preflight.json
RUN_ROOT=/models/.abliteration/k3/v13-calibration-run-v1
...
die() {
    echo "run_v13_calibration_server: $*" >&2
    exit 1
}
...
verify_v2_inventory() {
    ...
    inventory_tmp=$(mktemp /tmp/k3-v13-v2-inventory.XXXXXX)
...
if [[ $# -eq 4 && "$1" == --preflight-only ]]; then
    ...
    die "usage: $0 [--preflight-only prompt13 PHASE --no-response] | prompt13 PHASE | --check-production-only"
fi
...
# The immutable V10 gate core derives this compatibility tag for prompt13.
...
check_sha256 108e67a9059ff86fb6e37aa0047b764c09c69ba11987ed9c2994d81470db3b58 "$EVALUATOR"
...
check_sha256 ceb7f2c8b3f5b990d4686da4d6dbdbb6188596bf5a0d0e1dab1fe435dabaecb1 "$STATE_HELPER"
...
check_sha256 c8fe1eab1d03eb922daf221299c772cf84ec0dcf36f07f9a22edfe71923a8277 "$GATE_HELPER"
check_sha256 5dfc4d7c80999076b4d14b7faa5212ba95fac462abddb4fd1ee229025b319b59 "$GATE_CORE"
check_sha256 1cb5c5a3f35f012df4677d5630fb5f113bcd165b43dc3b21e00b4502667980c2 "$PROTOCOL"
check_sha256 6f52ae9400f1ae642d8d173905e3366492aefeeaa177eb9e171a2e7e181ee48a "$PRIOR_RESULTS"
check_sha256 0800c4cbe4bf5d169c4b57d53cc1b68b4534861c9555c870a4eaf5ec97d97c13 "$ENGINE_MANIFEST"
check_sha256 2a132fcd2392b2b8ec77ed763fd3cd0eff86fbcc5f305e4fbb32c3577532f897 "$PREFLIGHT_HELPER"
check_sha256 feec21c998f1845c75b75bf401384b432deb03edea01b29c75d035167a3c0f5f "$CONTROL_RECEIPT"
check_sha256 1b93736d8341ef04aacfdfd3a546945c07d19a821021b256d66e964f91c1ac36 "$PREFLIGHT_RECEIPT"
...
if [[ "$preflight_only" == 1 ]]; then
    echo "PASS V13 preflight prompt=$PROMPT phase=$PHASE; production remains active"
    exit 0
fi
