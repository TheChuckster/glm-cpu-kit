#!/usr/bin/env python3
"""Response-free proof for K3 V24 reasoning prefill plus no-colon DRY."""
...
def validate_dry_argv(argv, required):
    """Require the exact no-colon DRY tuple once and in order, or omit DRY."""
    inline = [
    ...
    if tuple(argv[start:start + len(DRY_ARGV)]) != DRY_ARGV:
        raise ValueError(
            "candidate no-colon DRY tuple is not contiguous and in registered order"
        )
...
def runtime_record(mode):
    ...
    if str(executable) != SERVER_PATH or sha256(executable) != SERVER_SHA256:
        raise ValueError("live server executable differs from the frozen V24 build")
...
def run_control(args, api_key, system_prompt, system_record, prefill_record):
    ...
    print(f"PASS V24 response-free control: {BASELINE_PROMPT_SHA256}")
...
def run_candidate(args, api_key, system_prompt, system_record, prefill_record):
    ...
    write_exclusive(args.output, receipt)
    print(
        f"PASS V24 response-free feature preflight: {EXTENDED_PROMPT_SHA256} "
