#!/usr/bin/env python3
"""Response-free proof for K3 V26 reasoning prefill plus quote-star DRY."""
...
SERVER_PATH = "/home/chuck/ik_llama-v26-78bdb309/build-v26/bin/llama-server"
SERVER_WORKING_DIRECTORY = "/home/chuck/ik_llama-v26-78bdb309"
MODEL_PATH = (
    "/models/Kimi-K3-Q5attn-Abliterated-V2/"
    "Kimi-K3-Q5attn-Abliterated-V2-00001-of-00019.gguf"
)
SERVER_SHA256 = "d65ea913f2bab718729b866b3d02c749ed93f6247d3a18e1807be7b617e95c7c"
ENGINE_COMMIT = "78bdb3092f1cb1cd9a95018aa0ff275ffa3f982a"
...
def validate_dry_argv(argv, required):
    """Require the exact quote-star DRY tuple once and in order, or omit DRY."""
    inline = [
    ...
    if inline:
        raise ValueError(f"DRY options must use separate argv values: {inline!r}")
    if not required:
    ...
    if tuple(argv[start:start + len(DRY_ARGV)]) != DRY_ARGV:
        raise ValueError(
            "candidate quote-star DRY tuple is not contiguous and in registered order"
        )
...
def runtime_record(mode):
    ...
    if str(executable) != SERVER_PATH or sha256(executable) != SERVER_SHA256:
        raise ValueError("live server executable differs from the frozen V26 build")
...
def load_control_receipt(path, system_prompt_record, prefill_record):
    ...
    if receipt.get("schema") != "k3-v26-response-free-control-v1":
...
def run_control(args, api_key, system_prompt, system_record, prefill_record):
    ...
    receipt = {
        "schema": "k3-v26-response-free-control-v1",
    ...
    print(f"PASS V26 response-free control: {BASELINE_PROMPT_SHA256}")
...
def run_candidate(args, api_key, system_prompt, system_record, prefill_record):
    ...
    receipt = {
        "schema": "k3-v26-response-free-preflight-v1",
        "captured_utc": datetime.now(timezone.utc).isoformat(),
        "base_url": args.base_url,
    ...
    write_exclusive(args.output, receipt)
    print(
        f"PASS V26 response-free feature preflight: {EXTENDED_PROMPT_SHA256} "
