#!/usr/bin/env bash
# Prove V19's exact control/feature templates without generating a model token.
set -euo pipefail
umask 077
...
RUN_ROOT=/models/.abliteration/k3/v19-response-free-preflight-v1
CONTROL_RECEIPT=$RUN_ROOT/control.json
PREFLIGHT_RECEIPT=$RUN_ROOT/preflight.json
PRODUCTION=glm-server.service
CONTROL_UNIT=kimi-k3-q5attn-abl-v19-control-preflight.service
CANDIDATE_UNIT=kimi-k3-q5attn-abl-v19-feature-preflight.service
CONTROL_ALIAS=kimi-k3-q5attn-abl-v19-control-preflight
CANDIDATE_ALIAS=kimi-k3-q5attn-abl-v19-ttf-preflight
...
die() {
    echo "run_v19_response_free_preflight: $*" >&2
    exit 1
}
...
if [[ $# -eq 1 && "$1" == --verify-files-only ]]; then
    ...
    verify_files
    echo "PASS V19 response-free file/test verification; production remains active"
    exit 0
fi
...
echo "COMPLETE V19 response-free preflight; restoring accepted V1"
