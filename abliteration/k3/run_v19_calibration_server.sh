#!/usr/bin/env bash
# Run one fixed V19 TTF phase PID and always restore accepted V1 production.
set -euo pipefail
umask 077
...
TOOLS=/models/.abliteration/k3/eval-tools-v19-v1
STATE_HELPER=$TOOLS/verify_v19_calibration_state.py
STATE_CORE=$TOOLS/verify_v10_calibration_state.py
EVALUATOR=$TOOLS/evaluate_reasoning_prefill_api_v19.py
...
GATE_HELPER=$TOOLS/gate_v19_calibration.py
GATE_CORE=$TOOLS/gate_v10_calibration.py
PROTOCOL=$TOOLS/V19_PROTOCOL.md
PRIOR_RESULTS=$TOOLS/V18_RESULTS.md
REASONING_PREFILL=$TOOLS/v19-reasoning-prefill.txt
ENGINE_MANIFEST=$TOOLS/v19-engine-sources.sha256
PREFLIGHT_HELPER=$TOOLS/preflight_v19_reasoning_prefill.py
RESPONSE_FREE_LAUNCHER=$TOOLS/run_v19_response_free_preflight.sh
PREFLIGHT_ROOT=/models/.abliteration/k3/v19-response-free-preflight-v1
CONTROL_RECEIPT=$PREFLIGHT_ROOT/control.json
PREFLIGHT_RECEIPT=$PREFLIGHT_ROOT/preflight.json
RUN_ROOT=/models/.abliteration/k3/v19-calibration-run-v1
...
die() {
    echo "run_v19_calibration_server: $*" >&2
    exit 1
}
...
verify_v2_inventory() {
    ...
    inventory_tmp=$(mktemp /tmp/k3-v19-v2-inventory.XXXXXX)
...
if [[ $# -eq 4 && "$1" == --preflight-only ]]; then
    ...
    die "usage: $0 [--preflight-only prompt19 PHASE --no-response] | prompt19 PHASE | --check-production-only"
fi

case "$PROMPT" in
    prompt19)
        ALIAS=kimi-k3-q5attn-abl-v19-ttf-cal
...
# The immutable V10 gate core derives this compatibility tag for prompt19.
...
check_sha256 3e280106d163c2f3cf2699ddace400fc3464a7ea01a12881562cb836c4fbab08 "$EVALUATOR"
...
check_sha256 1c5ad6592996f0e43f15d94905bae87e456f33472e8dba6e321d5aec116088c0 "$STATE_HELPER"
...
check_sha256 8edce1dd01f14ed024798d64d29fcb83ce922fc2ddd8d7c27a3a22e7273fbdf3 "$GATE_HELPER"
check_sha256 5dfc4d7c80999076b4d14b7faa5212ba95fac462abddb4fd1ee229025b319b59 "$GATE_CORE"
check_sha256 6e79ae83d92787509200eeb1c8c8de494d10866fa8d22153b8f1222eca8ccd4e "$PROTOCOL"
check_sha256 3e75f480deca16382eef1cc3ec1f41a53960a32f7ad93567595704e40b9d5abf "$PRIOR_RESULTS"
check_sha256 0800c4cbe4bf5d169c4b57d53cc1b68b4534861c9555c870a4eaf5ec97d97c13 "$ENGINE_MANIFEST"
check_sha256 5e80a81e926ce2db612e37832df5213a819bc6848f5922209e4c16c27b01f500 "$PREFLIGHT_HELPER"
check_sha256 72d6559888401da4366f705dc055397a31f135a2f7c62ac15e54f4e36978b741 "$RESPONSE_FREE_LAUNCHER"
check_sha256 42465dc5a095fe182bc19165ff7fa536b4592e1d723b12112179192345932065 "$CONTROL_RECEIPT"
check_sha256 953581e20122da96d9819a91f73fbf890c1d834003f56faa04d62d45a0678418 "$PREFLIGHT_RECEIPT"
...
if [[ "$preflight_only" == 1 ]]; then
    echo "PASS V19 preflight prompt=$PROMPT phase=$PHASE; production remains active"
    exit 0
fi
