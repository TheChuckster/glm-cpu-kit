#!/usr/bin/env bash
# Run one fixed V21 canonical-DRY phase PID and restore accepted V1 production.
set -euo pipefail
umask 077
...
TOOLS=/models/.abliteration/k3/eval-tools-v21-v1
STATE_HELPER=$TOOLS/verify_v21_calibration_state.py
STATE_CORE=$TOOLS/verify_v10_calibration_state.py
EVALUATOR=$TOOLS/evaluate_reasoning_prefill_api_v21.py
...
GATE_HELPER=$TOOLS/gate_v21_calibration.py
GATE_CORE=$TOOLS/gate_v10_calibration.py
PROTOCOL=$TOOLS/V21_PROTOCOL.md
PRIOR_RESULTS=$TOOLS/V20_RESULTS.md
REASONING_PREFILL=$TOOLS/v21-reasoning-prefill.txt
ENGINE_MANIFEST=$TOOLS/v21-engine-sources.sha256
PREFLIGHT_HELPER=$TOOLS/preflight_v21_reasoning_prefill.py
RESPONSE_FREE_LAUNCHER=$TOOLS/run_v21_response_free_preflight.sh
PREFLIGHT_ROOT=/models/.abliteration/k3/v21-response-free-preflight-v1
CONTROL_RECEIPT=$PREFLIGHT_ROOT/control.json
PREFLIGHT_RECEIPT=$PREFLIGHT_ROOT/preflight.json
RUN_ROOT=/models/.abliteration/k3/v21-calibration-run-v1
...
die() {
    echo "run_v21_calibration_server: $*" >&2
    exit 1
}
...
verify_v2_inventory() {
    ...
    inventory_tmp=$(mktemp /tmp/k3-v21-v2-inventory.XXXXXX)
...
if [[ $# -eq 4 && "$1" == --preflight-only ]]; then
    ...
    die "usage: $0 [--preflight-only prompt21 PHASE --no-response] | prompt21 PHASE | --check-production-only"
fi

case "$PROMPT" in
    prompt21)
        ALIAS=kimi-k3-q5attn-abl-v21-dry-ttf-cal
...
# The immutable V10 gate core derives this compatibility tag for prompt21.
...
check_sha256 94eba25a3eaece9a534fe793420910a76584e2610559cc36e0dccfbfed6d98d7 "$EVALUATOR"
...
check_sha256 97967d718c11f35d9fc523e1573c6951d91e0ee2ddac7f97034f6b9a0c40d59b "$STATE_HELPER"
...
check_sha256 21b63b8914c7530be1ed7ff399e8b82b04a8f0af3885190fedd1ce28689fd539 "$GATE_HELPER"
check_sha256 5dfc4d7c80999076b4d14b7faa5212ba95fac462abddb4fd1ee229025b319b59 "$GATE_CORE"
check_sha256 fe05784930d31fde8306132f3862227f41b15118043577d1321fab66369ae1d2 "$PROTOCOL"
check_sha256 07bfabf4b5736f84b3a043799c9a14e66c017f11151245889583ae97e3af1afb "$PRIOR_RESULTS"
check_sha256 04b9aab1c52ba0500731fd39f7c48d0d0b2def7356c495fa46fc202fb2555693 "$ENGINE_MANIFEST"
check_sha256 7dd2512b031b5ed0342bb3e025e31cf5b743dd77452690b1ea79ad5614d7037b "$PREFLIGHT_HELPER"
check_sha256 a018fe8d5bfc36a135d5cd391cbb310ccfaae88222474f35a78ead64d12cc641 "$RESPONSE_FREE_LAUNCHER"
check_sha256 a3acd6e22b43e37930b2bea14a753429bd5722c387d3b9d123ddb62341fb7829 "$CONTROL_RECEIPT"
check_sha256 ee989366f4dd74f049099d920634250e3d228f35bac864655f6ffaac5869f181 "$PREFLIGHT_RECEIPT"
...
if [[ "$preflight_only" == 1 ]]; then
    echo "PASS V21 preflight prompt=$PROMPT phase=$PHASE; production remains active"
    exit 0
fi
