#!/usr/bin/env bash
# Run one fixed V15 TTF phase PID and always restore accepted V1 production.
set -euo pipefail
umask 077
...
TOOLS=/models/.abliteration/k3/eval-tools-v15-v1
STATE_HELPER=$TOOLS/verify_v15_calibration_state.py
STATE_CORE=$TOOLS/verify_v10_calibration_state.py
EVALUATOR=$TOOLS/evaluate_reasoning_prefill_api_v15.py
...
GATE_HELPER=$TOOLS/gate_v15_calibration.py
GATE_CORE=$TOOLS/gate_v10_calibration.py
PROTOCOL=$TOOLS/V15_PROTOCOL.md
PRIOR_RESULTS=$TOOLS/V14_RESULTS.md
REASONING_PREFILL=$TOOLS/v15-reasoning-prefill.txt
ENGINE_MANIFEST=$TOOLS/v15-engine-sources.sha256
PREFLIGHT_HELPER=$TOOLS/preflight_v15_reasoning_prefill.py
RESPONSE_FREE_LAUNCHER=$TOOLS/run_v15_response_free_preflight.sh
PREFLIGHT_ROOT=/models/.abliteration/k3/v15-response-free-preflight-v1
CONTROL_RECEIPT=$PREFLIGHT_ROOT/control.json
PREFLIGHT_RECEIPT=$PREFLIGHT_ROOT/preflight.json
RUN_ROOT=/models/.abliteration/k3/v15-calibration-run-v1
...
die() {
    echo "run_v15_calibration_server: $*" >&2
    exit 1
}
...
verify_v2_inventory() {
    ...
    inventory_tmp=$(mktemp /tmp/k3-v15-v2-inventory.XXXXXX)
...
if [[ $# -eq 4 && "$1" == --preflight-only ]]; then
    ...
    die "usage: $0 [--preflight-only prompt15 PHASE --no-response] | prompt15 PHASE | --check-production-only"
fi

case "$PROMPT" in
    prompt15)
        ALIAS=kimi-k3-q5attn-abl-v15-ttf-cal
...
# The immutable V10 gate core derives this compatibility tag for prompt15.
...
check_sha256 6e457576266f98e64272c27e34e955f1d2eb3b30cafd8545f6dca02e7b8e4a84 "$REASONING_PREFILL"
check_sha256 19636d20c4e8d8ee1599e876f833681d2dcd192bfdb1c15b753d9c00e501bab4 "$EVALUATOR"
...
check_sha256 49834c75fe45bd3f95755c7f0efdd070203280bb009ee1fb0493771c8ec56a0d "$STATE_HELPER"
...
check_sha256 a9ee11c48468e06b01015b6860ebc3ae4913a67567a7f5bb6ca0ac6eac1ea74b "$GATE_HELPER"
check_sha256 5dfc4d7c80999076b4d14b7faa5212ba95fac462abddb4fd1ee229025b319b59 "$GATE_CORE"
check_sha256 8b132e020686942e5b825c576296048aa6917029e60a5cfcfb928509012f04be "$PROTOCOL"
check_sha256 a6866167b262e8b3d5b9c3f2be5be2982f8030c599e71aa68348384def96c406 "$PRIOR_RESULTS"
check_sha256 0800c4cbe4bf5d169c4b57d53cc1b68b4534861c9555c870a4eaf5ec97d97c13 "$ENGINE_MANIFEST"
check_sha256 c5843fa92236c901ce7d7b1b0697f6fcf3e39aa30b64ea473c1bf7e61d920304 "$PREFLIGHT_HELPER"
check_sha256 1a3402d80e750f84da3180d1138289abf3f7a90ef57d194ab1e9966b9ac9eb64 "$RESPONSE_FREE_LAUNCHER"
check_sha256 16e9dc2e09cab71062de033b07691ef6f3879c5baf9cdb37d16b1cd617dfe6b1 "$CONTROL_RECEIPT"
check_sha256 a85a89c36d0a9e0df80d6b5d2928094bd18f013950734c6cf61048ab77af6182 "$PREFLIGHT_RECEIPT"
...
if [[ "$preflight_only" == 1 ]]; then
    echo "PASS V15 preflight prompt=$PROMPT phase=$PHASE; production remains active"
    exit 0
fi
