#!/usr/bin/env python3
"""Focused response-free closure tests for V25 no-newline DRY."""
...
import gate_v25_calibration as gate
import preflight_v25_reasoning_prefill as preflight
import verify_v25_calibration_state as state
from evaluate_api import load_system_prompt
from evaluate_reasoning_prefill_api_v25 import (
...
class V25ConfigurationTests(unittest.TestCase):
    ...
    def test_exact_candidate_and_artifact_identity(self):
        ...
        expected = {
            "alias": "kimi-k3-q5attn-abl-v25-no-newline-dry-ttf-cal",
            "filename": self.prompt.name,
            "sha256": prompt_hash,
        }
        self.assertEqual(prompt_hash, gate.PROMPTS["prompt25"]["sha256"])
        self.assertEqual(prefill_hash, gate.REASONING_PREFILL_SHA256)
        self.assertEqual(
            self.prefill.read_bytes(),
            (gate.REASONING_PREFILL_TEXT + "\n").encode(),
        )
        self.assertEqual(gate.PROMPT_ORDER, ("prompt25",))
        self.assertEqual(gate.PROMPTS, {"prompt25": expected})
        self.assertEqual(state.PROMPTS, {"prompt25": expected})
        self.assertEqual(
            hashlib.sha256(
                (self.root / "evaluate_reasoning_prefill_api_v25.py").read_bytes()
            ).hexdigest(),
        ...
        self.assertEqual(
            hashlib.sha256(
                (self.root / "verify_v25_calibration_state.py").read_bytes()
            ).hexdigest(),
            gate.STATE_HELPER_SHA256,
        )
        self.assertEqual(
            hashlib.sha256(
                (self.root / "v25-engine-sources.sha256").read_bytes()
            ).hexdigest(),
    ...
    def test_engine_identity_and_source_manifest_are_frozen(self):
        ...
        records = {}
        for line in (self.root / "v25-engine-sources.sha256").read_text().splitlines():
        ...
        self.assertEqual(
            ...
            "92e7d18dc39ad4495ceb4890f013dcd23c406daad9dce2c1dac37357f354756a",
        )
    ...
    def test_all_engine_test_receipts_are_green(self):
        ...
        receipts = {
            "v25-local-normal-greedy-dry.xml": ("test-greedy-dry", "charbro"),
    ...
    def test_result_identity_contract_and_server_argv_are_frozen(self):
        ...
        argv = gate.expected_server_argv(gate.PROMPTS["prompt25"]["alias"])
        self.assertEqual(
    ...
    def test_dry_argv_is_exact_and_all_mutations_fail_closed(self):
        ...
        behavior = gate.expected_server_argv(gate.PROMPTS["prompt25"]["alias"])
    ...
    def test_effective_props_bind_control_and_feature_dry_settings(self):
        ...
        newline_breaker = body(
            preflight.CANDIDATE_ALIAS, preflight.FEATURE_EFFECTIVE_DRY_SAMPLER
        )
        newline_breaker["default_generation_settings"]["dry_sequence_breakers"] = [
            "\n", ":", "\"", "*",
        ]
        with self.assertRaisesRegex(ValueError, "effective DRY settings differ"):
            preflight.validate_props(
                200, newline_breaker, preflight.CANDIDATE_ALIAS,
                preflight.FEATURE_EFFECTIVE_DRY_SAMPLER,
            )
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            provenance = {
                ...
                "evaluation_protocol_artifacts": [
                    ...
                    gate.core.artifact_record(self.root / "v25-engine-sources.sha256"),
                ],
            }
    ...
    def test_prompt25_phase_unit_and_run_root_are_unique(self):
        launcher = (self.root / "run_v25_calibration_server.sh").read_text()
        response_free = (
            self.root / "run_v25_response_free_preflight.sh"
        ).read_text()
        self.assertIn("RUN_ROOT=/models/.abliteration/k3/v25-calibration-run-v1", launcher)
        self.assertIn("TOOLS=/models/.abliteration/k3/eval-tools-v25-v2", launcher)
        self.assertIn(
        ...
        self.assertIn(
            ...
            "v25-response-free-preflight-v1",
            launcher,
        )
        self.assertIn(
        ...
        self.assertNotIn("eval-tools-v25-v1", launcher)
        self.assertIn(
            "UNIT=kimi-k3-q5attn-abl-v10-${PROMPT}-${PHASE}-cal.service",
            launcher,
        )
        self.assertIn("prompt25)", launcher)
        self.assertIn(
            "ALIAS=kimi-k3-q5attn-abl-v25-no-newline-dry-ttf-cal", launcher
        )
