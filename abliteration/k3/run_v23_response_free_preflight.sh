#!/usr/bin/env bash
# Prove V23's exact control/feature templates without generating a model token.
set -euo pipefail
umask 077
...
die() {
    echo "run_v23_response_free_preflight: $*" >&2
    exit 1
}
...
if [[ $# -eq 1 && "$1" == --verify-files-only ]]; then
    ...
    verify_files
    echo "PASS V23 response-free file/test verification; production remains active"
    exit 0
fi
...
echo "COMPLETE V23 response-free preflight; restoring accepted V1"
