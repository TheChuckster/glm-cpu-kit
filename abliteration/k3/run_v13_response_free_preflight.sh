#!/usr/bin/env bash
# Prove V13's exact control/feature templates without generating a model token.
set -euo pipefail
umask 077
...
RUN_ROOT=/models/.abliteration/k3/v13-response-free-preflight-v1
...
die() {
    echo "run_v13_response_free_preflight: $*" >&2
    exit 1
}
...
start_candidate() {
    ...
    if [[ "$mode" == control ]]; then
        ...
        prefill_args=(--reasoning-prefill 'I know that.')
    fi
    candidate_started=1
    sudo -n systemd-run \
        --unit="$current_unit" \
...
if [[ $# -eq 1 && "$1" == --verify-files-only ]]; then
    ...
    verify_files
    echo "PASS V13 response-free file/test verification; production remains active"
    exit 0
fi
...
echo "COMPLETE V13 response-free preflight; restoring accepted V1"
