#!/usr/bin/env python3
"""Focused response-free closure tests for V26 quote-star DRY."""
...
import gate_v26_calibration as gate
import preflight_v26_reasoning_prefill as preflight
import verify_v26_calibration_state as state
from evaluate_api import load_system_prompt
from evaluate_reasoning_prefill_api_v26 import (
...
class V26ConfigurationTests(unittest.TestCase):
    ...
    def test_exact_candidate_and_artifact_identity(self):
        ...
        self.assertEqual(prompt_hash, gate.PROMPTS["prompt26"]["sha256"])
        self.assertEqual(prefill_hash, gate.REASONING_PREFILL_SHA256)
        self.assertEqual(
            self.prefill.read_bytes(),
            (gate.REASONING_PREFILL_TEXT + "\n").encode(),
        )
        self.assertEqual(gate.PROMPT_ORDER, ("prompt26",))
        self.assertEqual(gate.PROMPTS, {"prompt26": expected})
        self.assertEqual(state.PROMPTS, {"prompt26": expected})
        self.assertEqual(
            hashlib.sha256(
                (self.root / "evaluate_reasoning_prefill_api_v26.py").read_bytes()
            ).hexdigest(),
        ...
        self.assertEqual(
            hashlib.sha256(
                (self.root / "verify_v26_calibration_state.py").read_bytes()
            ).hexdigest(),
            gate.STATE_HELPER_SHA256,
        )
        self.assertEqual(
            hashlib.sha256(
                (self.root / "v26-engine-sources.sha256").read_bytes()
            ).hexdigest(),
    ...
    def test_engine_identity_and_source_manifest_are_frozen(self):
        ...
        records = {}
        for line in (self.root / "v26-engine-sources.sha256").read_text().splitlines():
        ...
        self.assertEqual(
            ...
            "5f543ed07210b37efeaab539ec550bd2538e78f2f8009076cc009351ed786ae5",
        )
    ...
    def test_all_engine_test_receipts_are_green(self):
        ...
        receipts = {
            "v26-local-normal-greedy-dry.xml": ("test-greedy-dry", "charbro"),
    ...
    def test_result_identity_contract_and_server_argv_are_frozen(self):
        ...
        argv = gate.expected_server_argv(gate.PROMPTS["prompt26"]["alias"])
        self.assertEqual(
        ...
        breaker = argv.index("--dry-sequence-breaker")
        self.assertEqual(argv[breaker + 1].encode(), b'"*')
        self.assertEqual(argv.count("--dry-sequence-breaker"), 1)
    ...
    def test_dry_argv_is_exact_and_all_mutations_fail_closed(self):
        ...
        behavior = gate.expected_server_argv(gate.PROMPTS["prompt26"]["alias"])
        ...
        self.assertNotIn(preflight.DRY_SEQUENCE_BREAKER_OPTION, control)
        for argv in (candidate, behavior):
            index = argv.index(preflight.DRY_SEQUENCE_BREAKER_OPTION)
            self.assertEqual(argv[index + 1], "\"*")
            self.assertNotIn(":", argv[index + 1])
            self.assertNotIn("\n", argv[index + 1])
        ...
        missing_breaker = candidate.copy()
        start = missing_breaker.index(preflight.DRY_SEQUENCE_BREAKER_OPTION)
        del missing_breaker[start:start + 2]
        default_breakers = candidate.copy()
        default_breakers[
            default_breakers.index(preflight.DRY_SEQUENCE_BREAKER_OPTION) + 1
        ] = "\n:\"*"
        colon_breaker = candidate.copy()
        colon_breaker[
            colon_breaker.index(preflight.DRY_SEQUENCE_BREAKER_OPTION) + 1
        ] = ":\"*"
        newline_breaker = candidate.copy()
        newline_breaker[
            newline_breaker.index(preflight.DRY_SEQUENCE_BREAKER_OPTION) + 1
        ] = "\n\"*"
        split_breakers = candidate.copy()
        start = split_breakers.index(preflight.DRY_SEQUENCE_BREAKER_OPTION)
        split_breakers[start:start + 2] = [
            preflight.DRY_SEQUENCE_BREAKER_OPTION, "\"",
            preflight.DRY_SEQUENCE_BREAKER_OPTION, "*",
        ]
        inline_breaker = candidate.copy()
        start = inline_breaker.index(preflight.DRY_SEQUENCE_BREAKER_OPTION)
        inline_breaker[start:start + 2] = ["--dry-sequence-breaker=\"*"]
        for name, argv in (
            ("missing", missing),
            ("duplicated", duplicated),
            ("reordered", reordered),
            ("altered", altered),
            ("duplicated_breaker", duplicated_breaker),
            ("missing_breaker", missing_breaker),
            ("default_breakers", default_breakers),
            ("colon_breaker", colon_breaker),
            ("newline_breaker", newline_breaker),
            ("split_breakers", split_breakers),
            ("inline_breaker", inline_breaker),
        ):
    ...
    def test_effective_props_bind_control_and_feature_dry_settings(self):
        ...
        default_breakers = body(
            preflight.CANDIDATE_ALIAS, preflight.FEATURE_EFFECTIVE_DRY_SAMPLER
        )
        default_breakers["default_generation_settings"]["dry_sequence_breakers"] = [
            "\n", ":", "\"", "*",
        ]
        with self.assertRaisesRegex(ValueError, "effective DRY settings differ"):
            preflight.validate_props(
                200, default_breakers, preflight.CANDIDATE_ALIAS,
                preflight.FEATURE_EFFECTIVE_DRY_SAMPLER,
            )
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            provenance = {
                ...
                "evaluation_protocol_artifacts": [
                    ...
                    gate.core.artifact_record(self.root / "v26-engine-sources.sha256"),
                ],
            }
    ...
    def test_prompt26_phase_unit_and_run_root_are_unique(self):
        launcher = (self.root / "run_v26_calibration_server.sh").read_text()
        response_free = (
            self.root / "run_v26_response_free_preflight.sh"
        ).read_text()
        self.assertIn("RUN_ROOT=/models/.abliteration/k3/v26-calibration-run-v1", launcher)
        self.assertIn("TOOLS=/models/.abliteration/k3/eval-tools-v26-v2", launcher)
        self.assertIn(
        ...
        self.assertIn(
            ...
            "v26-response-free-preflight-v1",
            launcher,
        )
        self.assertIn(
        ...
        self.assertNotIn("eval-tools-v26-v1", launcher)
        self.assertIn(
            "UNIT=kimi-k3-q5attn-abl-v10-${PROMPT}-${PHASE}-cal.service",
            launcher,
        )
        self.assertIn("prompt26)", launcher)
        self.assertIn(
            "ALIAS=kimi-k3-q5attn-abl-v26-quote-star-dry-ttf-cal", launcher
        )
        ...
        self.assertIn("--dry-sequence-breaker \"$dry_breakers\"", launcher)
        self.assertIn("dry_breakers='\"*'", launcher)
        self.assertIn("--dry-sequence-breaker \"$dry_breakers\"", response_free)
        self.assertIn("dry_breakers='\"*'", response_free)
    ...
    def test_launcher_literal_sha256_values_are_well_formed(self):
        ...
        self.assertNotIn(
            "PENDING_V26", (self.root / "gate_v26_calibration.py").read_text()
        )
        self.assertNotIn(
            "PENDING_V26", (self.root / "run_v26_calibration_server.sh").read_text()
        )
