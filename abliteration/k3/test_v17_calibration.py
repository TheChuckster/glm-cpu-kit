#!/usr/bin/env python3
"""Focused response-free closure tests for V17 Thought Token Forcing."""
...
import gate_v17_calibration as gate
import preflight_v17_reasoning_prefill as preflight
import verify_v17_calibration_state as state
from evaluate_api import load_system_prompt
from evaluate_reasoning_prefill_api_v17 import (
...
class V17ConfigurationTests(unittest.TestCase):
    def setUp(self):
        self.root = Path(__file__).parent
        self.prompt = self.root / "v10-system-prompt-02-semantic-contract.txt"
        self.prefill = self.root / "v17-reasoning-prefill.txt"
    ...
    def test_exact_candidate_and_artifact_identity(self):
        ...
        self.assertEqual(
            hashlib.sha256(
                (self.root / "evaluate_reasoning_prefill_api_v17.py").read_bytes()
            ).hexdigest(),
        ...
        self.assertEqual(
            hashlib.sha256(
                (self.root / "verify_v17_calibration_state.py").read_bytes()
            ).hexdigest(),
            gate.STATE_HELPER_SHA256,
        )
        self.assertEqual(
            hashlib.sha256(
                (self.root / "v17-engine-sources.sha256").read_bytes()
            ).hexdigest(),
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            provenance = {
                ...
                "evaluation_protocol_artifacts": [
                    ...
                    gate.core.artifact_record(self.root / "v17-engine-sources.sha256"),
                ],
            }
    ...
    def test_launcher_literal_sha256_values_are_well_formed(self):
        for filename in (
            "run_v17_response_free_preflight.sh",
            "run_v17_calibration_server.sh",
        ):
