#!/usr/bin/env bash
# Run one fixed V18 TTF phase PID and always restore accepted V1 production.
set -euo pipefail
umask 077
...
STATE_HELPER=$TOOLS/verify_v18_calibration_state.py
STATE_CORE=$TOOLS/verify_v10_calibration_state.py
EVALUATOR=$TOOLS/evaluate_reasoning_prefill_api_v18.py
...
GATE_HELPER=$TOOLS/gate_v18_calibration.py
...
RESPONSE_FREE_LAUNCHER=$TOOLS/run_v18_response_free_preflight.sh
PREFLIGHT_ROOT=/models/.abliteration/k3/v18-response-free-preflight-v1
CONTROL_RECEIPT=$PREFLIGHT_ROOT/control.json
PREFLIGHT_RECEIPT=$PREFLIGHT_ROOT/preflight.json
RUN_ROOT=/models/.abliteration/k3/v18-calibration-run-v1
...
die() {
    echo "run_v18_calibration_server: $*" >&2
    exit 1
}
...
if [[ $# -eq 4 && "$1" == --preflight-only ]]; then
    ...
    die "usage: $0 [--preflight-only prompt18 PHASE --no-response] | prompt18 PHASE | --check-production-only"
fi
...
# The immutable V10 gate core derives this compatibility tag for prompt18.
...
check_sha256 e54c19cf1d078f64335b3f531542267fda3d82155962ef4dcc5dfd9604665c90 "$EVALUATOR"
...
check_sha256 7a005c24bd1c6c6136deecf0e14d7ec6a0a1eddf6de2ae6f75879a7e19e4ed7a "$STATE_HELPER"
...
check_sha256 7c32ee73444e4b6c6e5fd18c5fd558466849baad131b132edac486d38493ee88 "$GATE_HELPER"
...
check_sha256 36a46cdc14dad91fc3c79a3b56ed15adc38631e7d2068f112e267748dec73039 "$RESPONSE_FREE_LAUNCHER"
check_sha256 87f58e3759135317b60a75a06885288f321b373a8c1d643b924a1306604b2ec0 "$CONTROL_RECEIPT"
check_sha256 c760a5a6714d174b566c0d86f3e0f8b4efff462dd7a467d36ddc7a5948911755 "$PREFLIGHT_RECEIPT"
...
if [[ "$preflight_only" == 1 ]]; then
    echo "PASS V18 preflight prompt=$PROMPT phase=$PHASE; production remains active"
    exit 0
fi
