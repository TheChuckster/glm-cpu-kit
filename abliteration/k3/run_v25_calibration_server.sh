#!/usr/bin/env bash
# Run one fixed V25 no-newline-DRY phase PID and restore accepted V1 production.
set -euo pipefail
umask 077
...
TOOLS=/models/.abliteration/k3/eval-tools-v25-v2
...
RESPONSE_FREE_LAUNCHER=$TOOLS/run_v25_response_free_preflight.sh
TEST_SUITE=$TOOLS/test_v25_calibration.py
PREFLIGHT_ROOT=/models/.abliteration/k3/v25-response-free-preflight-v1
CONTROL_RECEIPT=$PREFLIGHT_ROOT/control.json
PREFLIGHT_RECEIPT=$PREFLIGHT_ROOT/preflight.json
RUN_ROOT=/models/.abliteration/k3/v25-calibration-run-v1
...
die() {
    echo "run_v25_calibration_server: $*" >&2
    exit 1
}
...
if [[ $# -eq 4 && "$1" == --preflight-only ]]; then
    ...
    die "usage: $0 [--preflight-only prompt25 PHASE --no-response] | prompt25 PHASE | --check-production-only"
fi
...
# The immutable V10 gate core derives this compatibility tag for prompt25.
...
[[ "${observed_tools[*]}" == "${expected_tools[*]}" ]] \
    || die "V25 finalized tool-tree membership changed"
...
check_sha256 1ad5c374e3998923b5c5195f24bd9bc224c70bf1f09ade34fc6bb09addedc557 "$GATE_HELPER"
...
check_sha256 abccdee120fb3a6f1b7032ff4ff2923aaa5c80383183e54af48f4475e8cf2dbc "$RESPONSE_FREE_LAUNCHER"
check_sha256 80399e4a0fe2413e795524e7034a5ad0694551825193fb34196bee5c92228d6c "$TEST_SUITE"
check_sha256 99b9df575b04c0b0be84d804e4aee2f141e2952f7f3c4700b9bc677ff9531462 "$CONTROL_RECEIPT"
check_sha256 2bd8440cc64b1c5f6e22f5fd169892e72a2982b564a5031be9c429132f204ff8 "$PREFLIGHT_RECEIPT"
...
if [[ "$preflight_only" == 1 ]]; then
    echo "PASS V25 preflight prompt=$PROMPT phase=$PHASE; production remains active"
    exit 0
fi
