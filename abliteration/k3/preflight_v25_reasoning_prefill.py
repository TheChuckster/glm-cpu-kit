#!/usr/bin/env python3
"""Response-free proof for K3 V25 reasoning prefill plus no-newline DRY."""
...
ENGINE_COMMIT = "ecf7446e02e5a473c8f8316d201836532b707b21"
...
def validate_dry_argv(argv, required):
    """Require the exact no-newline DRY tuple once and in order, or omit DRY."""
    inline = [
    ...
    if tuple(argv[start:start + len(DRY_ARGV)]) != DRY_ARGV:
        raise ValueError(
            "candidate no-newline DRY tuple is not contiguous and in registered order"
        )
...
def runtime_record(mode):
    ...
    if str(executable) != SERVER_PATH or sha256(executable) != SERVER_SHA256:
        raise ValueError("live server executable differs from the frozen V25 build")
...
def load_control_receipt(path, system_prompt_record, prefill_record):
    ...
    if receipt.get("schema") != "k3-v25-response-free-control-v1":
...
def run_control(args, api_key, system_prompt, system_record, prefill_record):
    ...
    receipt = {
        "schema": "k3-v25-response-free-control-v1",
    ...
    print(f"PASS V25 response-free control: {BASELINE_PROMPT_SHA256}")
...
def run_candidate(args, api_key, system_prompt, system_record, prefill_record):
    ...
    receipt = {
        "schema": "k3-v25-response-free-preflight-v1",
        "captured_utc": datetime.now(timezone.utc).isoformat(),
        "base_url": args.base_url,
    ...
    write_exclusive(args.output, receipt)
    print(
        f"PASS V25 response-free feature preflight: {EXTENDED_PROMPT_SHA256} "
