#!/usr/bin/env python3
"""Focused response-free closure tests for V24 no-colon DRY."""
...
import gate_v24_calibration as gate
import preflight_v24_reasoning_prefill as preflight
import verify_v24_calibration_state as state
from evaluate_api import load_system_prompt
from evaluate_reasoning_prefill_api_v24 import (
...
class V24ConfigurationTests(unittest.TestCase):
    ...
    def test_exact_candidate_and_artifact_identity(self):
        ...
        self.assertEqual(prompt_hash, gate.PROMPTS["prompt24"]["sha256"])
        self.assertEqual(prefill_hash, gate.REASONING_PREFILL_SHA256)
        self.assertEqual(
            self.prefill.read_bytes(),
            (gate.REASONING_PREFILL_TEXT + "\n").encode(),
        )
        self.assertEqual(gate.PROMPT_ORDER, ("prompt24",))
        self.assertEqual(gate.PROMPTS, {"prompt24": expected})
        self.assertEqual(state.PROMPTS, {"prompt24": expected})
        self.assertEqual(
            hashlib.sha256(
                (self.root / "evaluate_reasoning_prefill_api_v24.py").read_bytes()
            ).hexdigest(),
        ...
        self.assertEqual(
            hashlib.sha256(
                (self.root / "verify_v24_calibration_state.py").read_bytes()
            ).hexdigest(),
            gate.STATE_HELPER_SHA256,
        )
        self.assertEqual(
            hashlib.sha256(
                (self.root / "v24-engine-sources.sha256").read_bytes()
            ).hexdigest(),
    ...
    def test_engine_identity_and_source_manifest_are_frozen(self):
        ...
        records = {}
        for line in (self.root / "v24-engine-sources.sha256").read_text().splitlines():
        ...
        self.assertEqual(
            ...
            "a806a4d8e97c44a861a7dbe3351c4c9e3871563b525a3adc99dddb71b213316e",
        )
    ...
    def test_all_engine_test_receipts_are_green(self):
        ...
        receipts = {
            "v24-local-normal-greedy-dry.xml": ("test-greedy-dry", "charbro"),
            "v24-local-normal-reasoning-prefill.xml": (
                "test-reasoning-prefill", "charbro"
            ),
            "v24-local-asan-ubsan-greedy-dry.xml": (
                "test-greedy-dry", "charbro"
            ),
            "v24-local-asan-ubsan-reasoning-prefill.xml": (
                "test-reasoning-prefill", "charbro"
            ),
            "v24-remote-normal-greedy-dry.xml": (
                "test-greedy-dry", "chuckdancer"
            ),
            "v24-remote-normal-reasoning-prefill.xml": (
    ...
    def test_result_identity_contract_and_server_argv_are_frozen(self):
        ...
        argv = gate.expected_server_argv(gate.PROMPTS["prompt24"]["alias"])
        self.assertEqual(
    ...
    def test_dry_argv_is_exact_and_all_mutations_fail_closed(self):
        ...
        behavior = gate.expected_server_argv(gate.PROMPTS["prompt24"]["alias"])
    ...
    def test_effective_props_bind_control_and_feature_dry_settings(self):
        ...
        colon_breaker["default_generation_settings"]["dry_sequence_breakers"] = [
            "\n", ":", "\"", "*",
        ]
        with self.assertRaisesRegex(ValueError, "effective DRY settings differ"):
            preflight.validate_props(
                200, colon_breaker, preflight.CANDIDATE_ALIAS,
                preflight.FEATURE_EFFECTIVE_DRY_SAMPLER,
            )
    ...
    def test_prompt24_phase_unit_and_run_root_are_unique(self):
        launcher = (self.root / "run_v24_calibration_server.sh").read_text()
        response_free = (
        ...
        self.assertIn("prompt24)", launcher)
        self.assertIn(
            "ALIAS=kimi-k3-q5attn-abl-v24-no-colon-dry-ttf-cal", launcher
        )
