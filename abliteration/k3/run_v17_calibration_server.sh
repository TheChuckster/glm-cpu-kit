#!/usr/bin/env bash
# Run one fixed V17 TTF phase PID and always restore accepted V1 production.
set -euo pipefail
umask 077
...
TOOLS=/models/.abliteration/k3/eval-tools-v17-v1
STATE_HELPER=$TOOLS/verify_v17_calibration_state.py
STATE_CORE=$TOOLS/verify_v10_calibration_state.py
EVALUATOR=$TOOLS/evaluate_reasoning_prefill_api_v17.py
...
GATE_HELPER=$TOOLS/gate_v17_calibration.py
GATE_CORE=$TOOLS/gate_v10_calibration.py
PROTOCOL=$TOOLS/V17_PROTOCOL.md
PRIOR_RESULTS=$TOOLS/V16_RESULTS.md
REASONING_PREFILL=$TOOLS/v17-reasoning-prefill.txt
ENGINE_MANIFEST=$TOOLS/v17-engine-sources.sha256
PREFLIGHT_HELPER=$TOOLS/preflight_v17_reasoning_prefill.py
RESPONSE_FREE_LAUNCHER=$TOOLS/run_v17_response_free_preflight.sh
PREFLIGHT_ROOT=/models/.abliteration/k3/v17-response-free-preflight-v1
CONTROL_RECEIPT=$PREFLIGHT_ROOT/control.json
PREFLIGHT_RECEIPT=$PREFLIGHT_ROOT/preflight.json
RUN_ROOT=/models/.abliteration/k3/v17-calibration-run-v1
...
die() {
    echo "run_v17_calibration_server: $*" >&2
    exit 1
}
...
verify_v2_inventory() {
    ...
    inventory_tmp=$(mktemp /tmp/k3-v17-v2-inventory.XXXXXX)
...
case "$PROMPT" in
    prompt16)
        ALIAS=kimi-k3-q5attn-abl-v17-ttf-cal
...
check_sha256 54d5fc374488dbb0557e82ee5b68f6c9cb6df19e68485ccd49f1d334e2ba461d "$EVALUATOR"
...
check_sha256 c9e118f05336042a81672a5c1014a3189f088cb796a96bcb2ac10dadcf4bc06c "$STATE_HELPER"
...
check_sha256 ce9b63a679aa91b5e0e9e98357832fa37748530ecc0e87adcf3eee2103984412 "$GATE_HELPER"
check_sha256 5dfc4d7c80999076b4d14b7faa5212ba95fac462abddb4fd1ee229025b319b59 "$GATE_CORE"
check_sha256 77542ff052c4afb1e2852de25e40b000ee15dc55a03efe938b251cd42ba46401 "$PROTOCOL"
check_sha256 930791ff24334f79c645b48919e2dcfa39d7c4e4aef7f04aa420a1eb0a8b9b63 "$PRIOR_RESULTS"
check_sha256 0800c4cbe4bf5d169c4b57d53cc1b68b4534861c9555c870a4eaf5ec97d97c13 "$ENGINE_MANIFEST"
check_sha256 cb1dc648879156c36a84aa0a1faad9cb598cd7d239c4475b3d713c8066341848 "$PREFLIGHT_HELPER"
check_sha256 de8810b4f292b40cc7d0d24aa1f83e15cd54f79082fb80e193b453c83b5c0efa "$RESPONSE_FREE_LAUNCHER"
check_sha256 43cefd3265239d820fefee268dd4b67fe0ff547adbc84cef6437783d20b58608 "$CONTROL_RECEIPT"
check_sha256 d12d624f9fdedd60703de7c317994f1b148162246ada326ecde4cb9ca733de5f "$PREFLIGHT_RECEIPT"
...
if [[ "$preflight_only" == 1 ]]; then
    echo "PASS V17 preflight prompt=$PROMPT phase=$PHASE; production remains active"
    exit 0
fi
